...
def inp_data(data_name):
    ...
    return tok_dev_data, tok_test_data, tok_train_data, conllu_dev_data, conllu_test_data, conllu_train_data, tok_vocabulary, conllu_vocabulary, tagset, dev_c, test_c
...
for i, (tokens,tags) in enumerate(conllu_train_set):
    train_texts.append(tokens)
    train_tags.append(tags)

for i, (tokens,tags) in enumerate(conllu_dev_set):
    dev_texts.append(tokens)
    dev_tags.append(tags)

for i, (tokens,tags) in enumerate(conllu_test_set):
...
PATH = os.path.join(sys.argv[1] + '_conllu_trained.pth')
...
if 'pdtb' in sys.argv[1]:
    def print_test_output():
        output_dir = "./outputs"
        final_test_file = os.path.join(output_dir, sys.argv[1] + '_test.conllu.preds')
    ...
    def print_dev_output():
        output_dir = "./outputs"
        final_dev_file = os.path.join(output_dir, sys.argv[1] + '_dev.conllu.preds')
